...
export default function Slider() {
  ...
  const [ scroll, setScroll ] = useState<{}>({transform: String, transition: String});
  ...
  useEffect(() => {
    setScroll({
      transform:  `translate(${window.innerWidth < 1024 ? `${mobileMeasure}px` : `${desktopMeasure}px`})`,
      transition: '.7s',
    })
  ...
  return(
    <div className={styles.slider}>
      <aside><i onClick={() => scrollLeft()} className='fas fa-chevron-left'></i></aside>
      <main>
      ...
      <aside><i onClick={() => scrollRight()} className='fas fa-chevron-right'></i></aside>
    </div>
  );
}